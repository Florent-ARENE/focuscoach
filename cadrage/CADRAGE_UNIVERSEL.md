...
Tout projet fournit un module `modules/diagnostic/` : un **hub** + des **pages de test** autonomes (santé runtime, contrôle de config, alignement version/doc/données, consoles API en **lecture seule**, smoke navigateur). **Protégé par auth, LECTURE SEULE** : aucune action destructive, **aucun effet de bord en GET** (un réglage de *session* type `SET time_zone` reste toléré — ce n'est pas une mutation de données), **secrets toujours masqués** (jamais une clé/token/mot de passe dans le HTML, **même tronqué**). **Résilient sans `die()`** : le module ne traverse **jamais** un chemin qui `die()`/`exit()` de façon non rattrapable (singleton de connexion, accès config global) — il interroge en **try/catch local** et **dégrade en « non vérifiable »** au lieu de planter. *L'outil qui diagnostique une panne ne doit pas mourir avec elle* (né du contournement d'un `getInstance()` qui `die()`). **Un seul design system** : écrit **exclusivement en tokens CSS**, zéro valeur en dur (couleur/hex) hors **un unique bloc de fallback délimité** (`var(--token, défaut)` neutre sobre) — garde-fou exécutable (AD-8), **étendu à tout nouveau fichier CSS du module** ; **jamais deux feuilles concurrentes ni détection JS du thème** (la cascade CSS suffit). **Zéro duplication** (AD-3) : auth, rendu des cartes `ok`/`attention`/`bloquant` et masquage des secrets vivent dans un helper commun, réutilisé par toutes les pages. Les détections (config, alignement) **dérivent d'une source unique** (AD-2) partagée avec les garde-fous build-time ; l'alignement couvre les stamps de version, les enums de statut, la cohérence catalogue **et les affirmations de doc devenues fausses** (référence à un mécanisme ou fichier inexistant), **y compris la cohérence de version du socle lui-même** (CADRAGE ↔ INSTRUCTIONS coordonnés, corps ↔ stamp) : le cadrage anti-dérive ne doit pas pouvoir dériver en silence. **Vérifié dans les 2 sens** (AD-9/AD-10) : chaque garde prouve qu'il **lève** sur anomalie **et** se **tait** quand tout va bien — protocole : *injecter l'anomalie → constater la détection → restaurer → vérifier le retour au vert* (`git status` propre). **Contrainte hébergement mutualisé** : un test qui interroge les surfaces du projet le fait **côté navigateur** (fetch client / iframe), pas en boucle serveur→serveur (loopback imbriqué non fiable). *Recoupe AD-8 (pendant runtime des garde-fous) : les **checks profonds** de santé y deviennent des **pages du module** (sous auth) ; le **liveness bon marché** reste un **endpoint léger à token** pour le monitoring externe (cf. AD-8).*
...
- [ ] Module `modules/diagnostic/` : auth, **lecture seule**, **secrets masqués**, **résilient sans `die()`**, alignement (version/doc/socle), **vérifié dans les 2 sens** (AD-11)
