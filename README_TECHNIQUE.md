...
**Version:** 2.10.0  
**Dernière mise à jour:** 11 juillet 2026 — v2.10.0 (validation des RDV unifiée + pilotable : même traitement séance simple ET forfait, réglage admin)
...
> ✅ **Source UNIQUE du changelog (AD-2, CLAUDE.md §8.1 « mémoire vivante »).** Le `CHANGELOG.md` racine est **gelé à 2.8.0** et redirige ici — ne pas tenir deux journaux. Toute nouvelle version s'inscrit **uniquement** dans cette table.

| Date | Version | Type | Description |
|------|---------|------|-------------|
| 11/07/2026 | 2.10.0 | ✅ Validation RDV unifiée | **Statut initial d'un RDV = UNE seule règle, pilotable en admin, IDENTIQUE séance simple ET forfait.** Avant : séance simple bypass → `pending` (validation), séance forfait → `confirmed` d'office (divergence = duplication de comportement). Désormais **`Booking::initialStatus()`** (source unique) lit le réglage **`settings.booking_auto_confirm`** (`'1'` défaut = confirmation auto, `'0'` = validation admin) et est appliquée à `create()` **et** `createFromPackage()` → même traitement pour chaque créneau, y compris chaque séance d'un forfait 5 jetons. **Toggle admin** « Confirmer automatiquement les rendez-vous » dans /admin → Paramètres (`booking_auto_confirm` ajouté à `$allowedKeys` + `saveSettings()` + libellé live). **Refund jeton sur refus** : annuler en admin une séance forfait encore vivante (`pending`/`confirmed`) rend le jeton au client (`Package::refundCredit`, symétrique du refund créneau-pris de pack-book). **Admin** : colonne Service affiche « (forfait N/total) » (ex. `1/5`) via `getAll()` enrichi (`pkg_index` sous-requête + `pkg_total` JOIN). Nouveau défaut `seed.sql`. Vérifié `[O1]` chaîne complète : réglage 0 → séance forfait `pending`, admin `1/5`, refus → jeton rendu (used 1→0), `php -l` 3/3, `node --check` admin.js, smoke 50/50. **Limite documentée** : le retour de paiement **Stripe** (`confirmByStripeSession`) confirme toujours sur paiement — quand Stripe sera activé (dormant, bypass partout), y brancher la même règle + la branche email correspondante. |
| 11/07/2026 | 2.9.2 | 📧 Mails cohérents | **Lien « Voir tout mon espace » ajouté dans tous les mails de réservation, via le bloc mutualisé `Mailer::addManageBlock()`.** Un seul point de code modifié → cohérence automatique sur : nouvelle demande (visiteur), confirmation, déplacement admin, demande de déplacement client. Le bloc contient maintenant 2 liens : « Gérer ce rendez-vous » (`manage.php?token=`) **et** « Voir tout mon espace » (`espace.php?token=`, même token — `espace.php` résout l'email → agrège séances + forfaits + historique). Les mails d'annulation/refus continuent d'inviter à reprendre RDV (rien à gérer) ; les mails forfait pointaient déjà vers `espace.php` / `pack.php`. Revue de cohérence globale des 9 mails effectuée. Vérifié `[O1]` : rendu du bloc (2 liens, token propagé), `php -l` OK, smoke 50/50. Rappel : contenu des mails = exception emoji tolérée (hors DOM, AD-5). |
| 11/07/2026 | 2.9.1 | 📘 Doc SQL | **`sql/TIPS.md` — runbook SQL d'exploitation.** Regroupe les manips SQL courantes découvertes en déploiement du site de test : débloquer le login admin (`DELETE FROM admin_login_attempts` — le rate-limit 5/15 min est vérifié AVANT le mot de passe, donc il masque tout), repartir de zéro (`DROP` + réimport `schema`/`seed`, renvoie vers [`reset-dev.sql`](sql/reset-dev.sql)), libérer un hold Stripe coincé, état des forfaits/jetons d'un client, contrôle d'alignement forfait→service, renseigner un `stripe_price_id`, purge des données de démo, diagnostics divers. Chaque bloc destructif marqué ⚠️. Croisé avec `schema.sql` (noms de tables/colonnes réels). Complète — pas remplace — `schema.sql`/`seed.sql`/`reset-dev.sql`/`seed-demo.sql`. |
| 26/06/2026 | 2.9.1 | 👤 Espace client unifié | **Tableau de bord client UNIFIÉ (`espace.php`) — cohérence + fidélisation, SANS rien casser.** Un client cumulant des séances à l'unité ET des forfaits avait des liens fragmentés (`manage.php?token=` par résa, `pack.php?token=` par forfait). Nouveau **`modules/booking/espace.php`** = vue d'ensemble **agrégée par email** : ses **forfaits** (jetons restants), ses **séances à venir**, son **historique** + compteur « séances réalisées » (levier fidélité). **Réutilise les tokens existants** (n'importe quel lien du client — résa ou forfait — résout son email → agrège) → **zéro friction, aucun compte, marche en local**. **N'AJOUTE que de la cohérence** : `manage.php`/`pack.php` restent intacts (actions par-item) et gagnent un lien « Voir tout mon espace » ; `espace.php` renvoie vers eux pour les actions (gérer/annuler une séance, réserver avec un forfait). Données : `Booking::getByEmail()` + `Package::getPurchasesByEmail()` (agrégation, AD-3). Emails de forfait pointent désormais vers `espace.php` (landing global). Vérifié `[O1]` : agrégation (2 séances + 1 forfait même email), rendu (forfaits + à venir + historique + liens manage/pack), cross-lien depuis pack, `php -l` 5/5, smoke 50/50, `bv3-help-note` (0 style inline). Mobile-first + PWA + a11y (réutilise coquille tunnel + cartes `manage.css`). |
| 26/06/2026 | 2.9.0 | 🎟️ §7 Forfaits/jetons | **Module FORFAITS À JETONS livré (§7) — acheter un forfait de N séances, puis les poser une par une (1 séance = 1 jeton).** Schéma déjà en place (`packages`, `package_purchases`, `bookings.package_purchase_id`) ; ce jalon apporte tout le code. **Modèle** `classes/Package.php` : `getActivePackages`, `createPurchase` (paymentMode-aware : `stripe`→`pending_payment` / `bypass`→`active`), `getByToken` (+ `credits_remaining`/`is_usable`), `attach/activateByStripeSession`, **`consumeCredit` (décrément ATOMIQUE** + garde `exhausted`/`expired` ; bug attrapé au test : `status` assigné AVANT `credits_used` en SQL sinon `exhausted` un cran trop tôt), `refundCredit`, `expireStalePurchases`, **`isPurchaseUsable` (pure, source unique, testée)**. **Achat** : zone « Forfaits » **sous** « Séances à l'unité » sur l'étape 1 (3 tiers, différenciation nette, cartes liseré orange, mobile-first + a11y) → `pack-buy.php` (récap + nom/email → `createPurchase`). **Espace pack** `pack.php?token=…` : dashboard jetons restants/utilisés, statut, validité, séances, bouton « Réserver ». **Réservation par jeton** : `pack_context()` mutualisé, `date/slot` portent `pack=<token>` (bannière, `data-pack` sur le calendrier, liens → `pack-book.php`), `pack-book.php` = consomme 1 jeton (atomique) → `Booking::createFromPackage` (statut `confirmed`/`paid`/`package_purchase_id`, **même garde anti-double-booking**) → **refund si créneau pris**. **Emails** : `Mailer::notifyPackagePurchase` (lien espace pack) + `notifyPackageSession`. **RGPD** : `cron/purge-rgpd.php` étape 4 (anonymise PII des achats expirés > 365 j, idempotent). **Admin** : onglet « Forfaits » (`?page=packages`) server-rendered (achats + jetons + statut, réutilise `.data-table`/`status-badge`). **Diagnostic** : `alignment` check « forfaits actifs → prestation active » (`health` couvrait déjà les tables). **AD-9** : `Lot 10` smoke (`isPurchaseUsable`, 6 cas 2 sens) → **smoke 50/50**. Vérifié `[O1]` : cycle de vie jetons (5→5→exhausted, over refusé, refund), résa par jeton end-to-end (booking `confirmed`/`paid`/`pkg_id`, anti-double-booking OK), rendus tunnel (date/slot/pack-book portent le pack), `php -l`, `node --check`, hex 0. **Mode `bypass`** (achat `active` direct, paiement hors-ligne) tant que les `stripe_price_id` des packages sont vides ; le path Stripe (Checkout + webhook forfait) est câblé côté achat, reste à finir côté webhook quand les produits existeront. Cadrage relu (AD-1→11) : aligné (AD-2 emails/rétention, AD-3 mutualisations, AD-4 tokens, AD-9 corpus, AD-11 diagnostic). |
| 26/06/2026 | 2.8.17 | 🔁 Reschedule client | **Déplacement de RDV côté CLIENT réactivé (`manage.php`), cohérent avec l'admin (2.8.16).** Le reschedule client était **volontairement désactivé depuis 2.6.0** (« contactez l'admin ») le temps que l'algo v3 se stabilise — §6 Stripe livré, condition levée. Le **backend était déjà prêt** (`Booking::clientReschedule()` + `api/manage.php?action=reschedule`) ; seule l'UI manquait. Ajout : bouton « Déplacer le rendez-vous » (si `service_id` connu, statut annulable) + **modale dédiée** (`#reschedule-modal`, même mécanisme `.modal-overlay`/`.active` que l'annulation) avec **date + sélecteur de créneau** chargé via `api/booking-v3-slots.php?service=X&date=Y` (**même endpoint service-aware que l'admin** → cohérence). `manage.js` : `showRescheduleModal`/`loadRescheduleSlots`/`confirmReschedule` (POST → `clientReschedule` qui remet en `pending` + email + garde anti-double-booking). Message « contactez renaud » retiré ; docblock manage.php corrigé (AD-7). **Vérifié `[O1]`** : `php -l`/`node --check` ✓, `clientReschedule` end-to-end (déplace+remet) OK, rendu (bouton + modale + date/select présents, ancien message retiré), smoke 44/44. **À tester au clic** : Déplacer → date → créneaux → Déplacer. |
| 26/06/2026 | 2.8.16 | 🐛 Fix reschedule | **Déplacement de RDV côté admin — « Erreur de chargement » des créneaux corrigée (2ᵉ résidu v2 révélé par les démos).** `admin.js` `loadAvailableSlots()` appelait `api/slots.php?date=…` — **endpoint supprimé à la purge 2.6.0** ; le v3 est `api/booking-v3-slots.php?service=X&date=Y`, **service-aware** (durée + buffer par prestation). Corrigé : `showRescheduleForm()` mémorise `this.state.rescheduleServiceId = booking.service_id`, `loadAvailableSlots()` interroge le bon endpoint avec le service. Forme de réponse identique (`{slots:[{time_start,time_end,label}]}`) → reste du flux inchangé. **Vérifié `[O1]`** : `node --check` ✓, endpoint v3 renvoie 20 créneaux (service 1, 30 juin) avec les bons champs, et **reschedule end-to-end OK** (`Booking::reschedule` déplace puis remet une démo, succès des 2 côtés — backend déjà v3, intact). ⚠️ **Reschedule CLIENT** (`manage.php`, via lien mail) reste **volontairement désactivé depuis 2.6.0** (le client annule + re-réserve) — `Booking::clientReschedule()` existe mais l'UI n'est pas branchée : c'est une **fonctionnalité à reconstruire**, pas ce bug. |
| 26/06/2026 | 2.8.15 | 🪟 Modale admin | **Les `confirm()` natifs du navigateur (« 127.0.0.1 indique… ») remplacés par une modale STYLÉE dans l'admin.** Nouveau `AdminApp.confirmModal({title, message, confirmLabel, danger})` → `Promise<boolean>`, réutilise le système `.modal-overlay`/`.modal` de main.css (même mécanisme `.active` que la modale client 2.8.12) + markup `#confirm-modal` dans `admin/index.php` (zéro CSS nouveau). Les **4 `confirm()`** d'`admin.js` (déplacer / supprimer / confirmer / refuser) passent à `await this.confirmModal(...)` ; bouton rouge `.btn-danger` pour les actions destructives (supprimer/refuser). Vérifs `[O1]` : `node --check` ✓, `php -l` ✓, **0 `confirm()` natif** réel (le restant est un commentaire), 4/4 appels, markup présent. **À tester au clic** : les 4 actions admin ouvrent/ferment la modale (fond, bouton Annuler). *(Reste : les `alert()` d'erreur de `manage.js`/`home.js` — pattern « notification », candidats à un toast, pas à une modale ; noté.)* |
| 26/06/2026 | 2.8.14 | 🐛 Fix admin | **Colonne « Service » vide dans l'admin (liste + détail) — corrigé.** `admin.js` lisait `b.service_label`, un **champ v2 supprimé à la purge 2.6.0** ; le v3 expose `service_name` (`s.name AS service_name` dans `Booking::getAll/getById`). Bug **invisible jusqu'ici faute de réservations** — les 5 RDV de démo (2.8.11) l'ont révélé. Corrigé 2× (`service_label` → `service_name || ''`, ligne liste + détail). Vérifié `[O1]` : `node --check` ✓, 0 résidu `service_label`, `getById()->service_name = "Déclic 30"`. *(Seul autre résidu `service_type` : `api/test-gcal.php` — string littérale dans un script de test, hors DOM, laissé.)* |
| 26/06/2026 | 2.8.13 | ♻️ Icône statut | **Mapping statut→icône centralisé dans `BOOKING_STATUS['icon']` (source unique PHP+JS) — dernier reliquat de l'audit.** Le champ `icon` de `BOOKING_STATUS` (jadis emoji déprécié non consommé) est **repurposé en nom Lucide** : `pending→hourglass`, `confirmed→circle-check`, `cancelled→circle-x`, `completed→check`. Consommé par **`admin.js`** (via `status_info.icon` exposé par `Booking::getById/getAll` — sa map en dur **supprimée**, `statusIcon()` lit désormais `status_info`) et **`manage.php`** (`$statusIcons` **dérivé** de `BOOKING_STATUS` + 2 transitionnels `pending_payment`/`expired` absents de la table, en fallback local documenté). `success.php` garde ses badges contextuels (pas un pur statut→icône). **Équivalence prouvée `[O1]`** : `status_info.icon` peuplé, map `manage` dérivé **==** l'ancien map en dur (zéro changement visuel), `php -l`/`node --check` ✓, smoke 44/44. Piège CLAUDE.md §10 corrigé (le champ `icon` est désormais **la** source, plus un emoji mort) ; note §2.3 `manage.css` mise à jour (contre-exemple **résolu**). |
| 26/06/2026 | 2.8.12 | 🪟 Modale unifiée | **Migration de la modale `manage` vers le mécanisme `.modal-overlay` de main.css → dernière collision CSS cross-feuilles FERMÉE.** La modale d'annulation de `manage.php` utilisait `.modal-overlay` avec un mécanisme **divergent** (affichée par défaut, masquée par `display:none` inline + JS) vs main.css (`visibility:hidden` par défaut, affichée par classe `.active`). Unifiée en 3 couches cohérentes : **manage.js** toggle `.active` (`showCancelModal`/`hideCancelModal`) au lieu de `style.display` ; **manage.php** retire le `style="display:none"` de `#cancel-modal` ; **manage.css** **supprime sa redéf `.modal-overlay`** (héritée de main.css) — seul le box `.modal-content` (propre, absent de main.css) reste. **Résultat : `grep` collisions cross-feuilles = VIDE** (plus aucune classe redéfinie entre 2 feuilles ; c'était le dernier point, après les boutons/badges/utilitaires en 2.8.7). Vérifs `[O1]` : `php -l` ✓, `node --check` ✓, hex manage.css 0, 3 couches cohérentes (toggle 2/2, 0 `display:none` sur la modale, `.modal-overlay.active` présent), `manage.php` (token démo) HTTP 200 avec modale présente. **À tester au clic (comportemental, non headless)** : ouverture/fermeture de la modale d'annulation. *(La modale RGPD `.delete-modal` garde son mécanisme propre — pas de collision, hors périmètre.)* |
| 26/06/2026 | 2.8.11 | 🌱 Seed démo | **`sql/seed-demo.sql` — 5 RDV d'exemple pour peupler l'admin en local (DEV uniquement).** Couvre les statuts `pending` / `confirmed` (dont **1 aujourd'hui**) / `cancelled` / `completed` / `pending_payment` (hold actif), prestations + visiteurs variés. **Idempotent** (purge d'abord les emails `@demo.focuscoach.test`, rejouable sans doublon), `SET NAMES utf8mb4`, `active_key` non insérée (générée). **⚠️ Strictement séparé de `seed.sql`** (qui reste la source de défauts PROD **sans fausses données**, AD-8) — ne jamais appliquer en prod. Vérifié `[O1]` : 5 RDV insérés, `Booking::getAll()` les renvoie avec `service_name` (JOIN OK → listing admin intact malgré les refactors 2.8.6), compteurs admin Aujourd'hui=1 / En attente=1 / Confirmées=1. |
| 26/06/2026 | 2.8.10 | 🧹 Code mort | **`assets/js/summary-module.js` supprimé (orphelin de la purge 2.6.0, AD-9.2).** L'audit (J1) le pointait comme « duplication du mapping statut→icône avec `admin.js` » ; vérification `[O1]` : il n'est **chargé par aucune page** (`<script>`), `SummaryModule` n'est **référencé nulle part**, seule trace = un commentaire de `icons.js` (qui citait aussi `booking.js`, déjà supprimé en 2.6.0). Donc pas une dup vivante mais du **code mort** survivant à la purge → supprimé (CLAUDE.md §9.2 « pas de fichier orphelin »). Commentaire `icons.js` corrigé (consommateurs réels : `admin.js`/`manage.js`/`booking-calendar.js`). Pas dans le précache `sw.js`. smoke 44/44. *Reste un mapping statut→icône en PHP (`manage.php` `$statusIcons`, `success.php` `$badge`) + JS (`admin.js`) sur des pages distinctes — centralisation possible via `BOOKING_STATUS['icon']` (champ existant, à repurposer), notée pour un passage dédié.* |
| 26/06/2026 | 2.8.9 | 🛡️ Garde footer | **Check « footer mutualisé » dans `alignment` (d5) — rend l'oubli 2.8.8 impossible en silence.** Nouveau parseur pur `diag_has_inline_footer($content)` + 5ᵉ ligne de la carte « Documentation, cadrage & pages » : itère les **9 pages publiques** (accueil + 2 légales + 6 booking) et **lève si une page déclare un `<footer>` EN DUR** au lieu de passer par `siteFooter()`/`booking_footer()` (le markup doit vivre dans le composant). C'est le garde qui aurait attrapé la dérive `.legal-footer` que l'humain a dû repérer à l'œil. Admin exclu (back-office, layout sidebar sans footer de site — légitime). Prouvé 2 sens `[O1]` (harnais 3/3 : rouge sur `<footer` en dur, vert sur composant, réel = 9 pages propres). Lecture seule, `ROOT_PATH`. smoke 44/44. *(Leçon 2.8.8 transformée en garde-fou automatique, pas juste un patch.)* |
| 26/06/2026 | 2.8.8 | 🎨 Footer légales | **Footer mutualisé sur les pages légales (incohérence repérée à l'œil, oubli de l'audit).** `mentions-legales.php` et `confidentialite.php` rendaient un footer minimal à part (`<footer class="legal-footer">` : © + 1 lien) au lieu du composant `siteFooter()` → footer « n'importe quoi », incohérent avec l'accueil. Corrigé : les 2 pages utilisent `<?= siteFooter(true) ?>` (footer du site navy complet, comme l'accueil) ; CSS mort `.legal-footer` retiré de `home.css`. **Inventaire footer COMPLET désormais** : accueil + 2 légales = `siteFooter(true)` ; 6 pages booking = `booking_footer()` (`siteFooter(false)`) ; `version.php` = utilitaire sans UI. Plus une seule page hors du composant unique. Vérifs `[O1]` : `php -l` ✓, 0 orphelin `.legal-footer` (hors commentaire), hex home.css 0, les 2 pages HTTP 200 avec `.site-footer` + colonnes Navigation/Informations. *(Leçon : l'audit doit énumérer TOUTES les pages, pas seulement celles touchées — 4 pages racine + 8 modules, c'est court.)* |
| 26/06/2026 | 2.8.7 | 🎨 Harmo CSS | **Dé-duplication CSS du design system (Partie D, volet visuel) — décisions d'intention validées par l'humain.** `manage.css` **ne redéfinit plus** le design system (AD-4) : `.btn`/`.btn-primary/secondary/danger` (+ `:hover`/`:disabled`), `.status-badge` (+ 4 variantes), `.text-muted`/`.text-center` **retirés** → **hérités de `main.css`** (source unique). Fini la dérive `.btn` `padding 0.75↔0.875`. **Badges** : `main.css` passe ses 4 variantes `.status-badge.*` aux **tokens sémantiques `--status-*`** (supprime un hex en dur `#b45309`/`#047857`/`#b91c1c` — AD-4) ; comme `manage` utilisait déjà ces tokens, **les couleurs ne bougent pas**, seul le gabarit s'aligne. **Collision `.stat-*` admin↔accueil** résolue : la **vitrine accueil** (gros chiffres + variantes couleur) renommée **`.metric-*`** (`home.css` + `index.php`) ; l'**admin garde `.stat-*`** (tableau de bord). **⚠️ `.modal-overlay` CONSERVÉ** dans `manage.css` (collision de nom mais **mécanisme différent** : main.css = `visibility:hidden`+classe d'état ; manage = `display:none` inline+JS) → le retirer casserait les modales ; unification = migration `manage.js` (dette tracée, **pas** une suppression CSS). Vérifs `[O1]` : hex modules **0** ; collisions cross-feuilles réduites à **`.modal-overlay`** (seul, documenté) ; **0 orphelin** `stat-` (accueil) ; accueil + manage HTTP 200 ; smoke 44/44. **À l'œil (humain)** : boutons manage au gabarit standard, badges au gabarit main.css (mêmes couleurs). Reste (D) : mapping statut→icône JS (`admin.js`↔`summary-module.js`) + migration modale manage. |
| 26/06/2026 | 2.8.6 | ♻️ Consolidation | **Dé-duplication de la logique anti-double-booking (Partie D de l'audit, volet logique) — équivalence PROUVÉE avant/après.** Le **prédicat « hold échu »** (`pending_payment` + `payment_expires_at < NOW()`), copié **6×**, devient SOURCE UNIQUE : `Booking::isHoldExpired(array,?now)` (miroir PHP, remplace les 2 copies inline `Slot::getActiveBookingsForDate` + `Booking::overlapsActive`) + constante `Booking::SQL_HOLD_EXPIRED` (fragment SQL, remplace les 4 copies dans `expireStaleHoldAt`/`expireStaleHolds`/`isSlotTaken`/`isSlotAvailable`). **Statuts actifs** `('pending','confirmed','pending_payment')` (4×) → `ACTIVE_BOOKING_STATUSES` (config) + `Booking::activeStatusesInSql()` ; le set **différent** `('pending','confirmed')` de `getBookingsByDateRange` est laissé tel quel (sémantique distincte — non fusionné). **Constantes magiques** : `HOLD_MINUTES` (15, hold Stripe — `createHold` + `process.php`), `MANAGE_TOKEN_LENGTH` (64, validations `strlen` ×5 **et** génération `bin2hex(random_bytes(LENGTH/2))`). **Preuve `[O1]`** : harnais 10/10 (`isHoldExpired` == ancien prédicat sur 4 cas ; fragments SQL byte-corrects ; `isSlotTaken`/`isSlotAvailable`/`getActiveBookingsForDate` **s'exécutent** sans erreur SQL ; UPDATE `prepare` valide) ; `grep` **0 littéral en dur restant** (hold 15, token 64, statut IN, fragment hold-échu) ; smoke 44/44 ; `php -l` 7/7. Reste (D) : CSS (`manage.css` redéfinit le design system, `.stat-*` divergents) + mapping statut→icône JS — traités au prochain passage (vérif visuelle). |
| 26/06/2026 | 2.8.5 | 🛡️ Garde-alignement | **Détection de dérive doc/cadrage câblée dans `modules/diagnostic/alignment.php` (Partie C de l'audit).** 4ᵉ carte « Documentation & cadrage » + 2 parseurs PURS dans `_diagnostic.php` (`diag_changelog_top_version`, `diag_spec_status` — testables hors fichier). Checks : **(d1)** `VERSION ↔ haut du changelog canonique` (README_TECHNIQUE) ; **(d2)** `CHANGELOG.md` = redirection (mono-source AD-2, bandeau présent) ; **(d3)** `spec ↔ versions livrées` (un §N dont le code proxy existe — ex. `StripeClient.php`→§6, `diagnostic/health.php`→§9 — doit être « livré », sinon dérive) ; **(d4)** exception inter-règles **AD-4↔AD-11** (`diagnostic.css` séparé) tracée dans CLAUDE.md §2.3. Chaque check **prouvé dans les 2 sens** `[O1]` (harnais : 11/11 — mord sur dérive injectée, se tait quand correct). Lecture seule, via `ROOT_PATH` (zéro chemin en dur → projet déplaçable, seul `config.local` se touche). **Résidus du cross-check corrigés** : `docs/booking-v3-spec.md` lignes 259/339 (« §6 à venir » hors tableau → livré + esquisse pré-livraison), CLAUDE.md règle 1 (carte des fonctions notée **gelée** d'ici le dégel v3.0.0). |
| 26/06/2026 | 2.8.4 | 🔒 Sécurité | **Gate cron RGPD durcie + mutualisée (faille secret en dur fermée).** `cron/purge-rgpd.php` (qui **supprime des données personnelles**) était gardé par un **secret en clair dans le code versionné** (`'CHANGEZ_CETTE_CLE_SECRETE_RGPD'`) + comparaison **non constante** (`!==`, fuite de timing) — viole RÈGLE 5.1. Trouvé à l'audit (sur preuve, pas relayé). Corrigé : gate **mutualisée** dans `Helpers::requireCronAuth(string $secret)` (CLI toujours OK ; HTTP = `hash_equals` temps constant + **fail-closed** si secret vide), appelée par **les deux** crons (élimine aussi la duplication divergente de la gate, AD-3). `purge-rgpd.php` lit désormais `RGPD_CRON_SECRET` depuis `config.local` (jamais en dur), bootstrap déplacé avant la gate ; `expire-holds.php` aligné sur le helper. Secret **dédié** documenté dans `config.local.php.template`. Preuve 2 sens `[O1]` : sans clé → **403**, ancienne clé en dur → **403** (accès ne passe plus), CLI inchangé. ⚠️ **Opérationnel** : si ce cron a déjà tourné en prod avec le placeholder, considérer la clé comme **compromise** → définir `RGPD_CRON_SECRET` et vérifier que l'URL refuse sans elle. **Réconciliation doc (même audit, AD-2/AD-7)** : `CHANGELOG.md` racine (resté à 2.8.0 ≠ `VERSION`) **gelé + redirigé** vers cette table (source unique) ; `docs/booking-v3-spec.md` corrigé (§6/§9 = **livré**, claims « à créer » obsolètes) ; **`api.js` fantôme retiré de `CLAUDE.md`** (3 réf. vers un fichier inexistant) ; **décision « `diagnostic.css` séparé = voulu » tracée** (CLAUDE.md §2.3). Reste à consolider (audit) : `manage.css` qui redéfinit le design system, `.stat-*` divergents, prédicat hold-échu ×6, constantes magiques `64`/`15`. |
| 26/06/2026 | 2.8.3 | ⚙️ Réglage | **Horizon de réservation porté de 60 à 180 jours (~6 mois).** `MAX_HORIZON_DAYS` (`config/config.php:99`) = 180. Impacte uniformément `Slot::getServiceAvailableDates` / `getServiceAvailabilityForMonth` / candidats (lecture seule, aucune autre logique touchée) → la liste **et** le calendrier (mois préc./suiv. bornés à `today+180 j`) couvrent désormais ~6 mois. Reste une **constante** (pas un réglage admin). Smoke 44/44. |
