/**
 * ============================================
 * ICONS — Miroir JS de classes/Icons.php
 * ============================================
 ...
 *   Icons.svg('eye', 18)        // taille custom
 *   Icons.svg('check', 20, 'icon-action')
 *
 * Pour ajouter une icône : la copier aussi dans classes/Icons.php
 * (sinon le rendu PHP et le rendu JS divergeront).
 */
...
const Icons = (function () {
    const LIBRARY = {
        ...
        // Diagnostic (module) — miroir de classes/Icons.php
    ...
    function svg(name, size, klass) {
        if (!LIBRARY[name]) return '';
        const s = size || 20;
        const c = klass || 'lucide';
        return `<svg xmlns="http://www.w3.org/2000/svg" width="${s}" height="${s}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="${c}" aria-hidden="true">${LIBRARY[name]}</svg>`;
    }
