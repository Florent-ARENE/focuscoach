...
/**
 * ============================================
 * CLASSE ICONS — Lucide SVG inline
 * ============================================
 * Bibliothèque minimaliste d'icônes Lucide (https://lucide.dev) embarquées
 * comme SVG inline. Avantages :
 *  - Pas de fetch externe (aucune dépendance réseau ni Lighthouse pénalisé).
 *  - Themable via `stroke="currentColor"` — la couleur suit `color: ...`.
 *  - Accessible : `aria-hidden="true"` par défaut (icônes décoratives).
 *
 * Usage PHP :
 *   <?= Icons::svg('mail') ?>
 *   <?= Icons::svg('arrow-right', 20, 'icon-cta') ?>
 *
 * Usage JS : voir assets/js/icons.js (miroir de cette bibliothèque).
 *
 * Ajout d'une icône : récupérer le SVG officiel sur https://lucide.dev,
 * ne garder que les enfants du <svg> et les coller dans LIBRARY ci-dessous.
...
class Icons
{
    private const LIBRARY = [
        ...
        // Statuts (pour mapping pending/confirmed/cancelled/completed)
        ...
        // Diagnostic (module) — miroir de assets/js/icons.js
    ...
    /**
     * Renvoie un SVG Lucide inline.
     *
     * @param string $name   Identifiant de l'icône (clé de LIBRARY).
     * @param int    $size   Taille du SVG carré, en pixels (par défaut 20).
     * @param string $class  Classe(s) CSS à ajouter (par défaut "lucide").
     * @return string  SVG prêt à insérer dans le HTML, ou '' si nom inconnu.
     */
    public static function svg(string $name, int $size = 20, string $class = 'lucide'): string
    {
